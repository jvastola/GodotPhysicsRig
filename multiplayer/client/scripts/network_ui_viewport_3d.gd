extends Node3D
## NetworkUIViewport3D - 3D world-space network UI panel

@export var pointer_group: StringName = &"pointer_interactable"
@export var ui_size: Vector2 = Vector2(800, 800)  # Match viewport size
...
func handle_pointer_event(event: Dictionary) -> void:
	...
	if debug_coordinates:
		print("NetworkUI Hit: global=", hit_pos, " local=", local_hit, " uv=", uv)
	...
	if debug_coordinates and event_type == "press":
		print("NetworkUI Viewport pos: ", viewport_pos)
...
func _world_to_uv(local_pos: Vector3) -> Vector2:
	...
	# while mesh local Y grows upward. Flip V by default.
	if flip_v:
		v = 1.0 - v
...
func set_interactive(enabled: bool) -> void:
	# Toggle visual and collision interactivity of the 3D UI.
	if mesh_instance:
