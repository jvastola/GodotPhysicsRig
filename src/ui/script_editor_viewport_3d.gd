...
# Script Editor Viewport 3D - 3D worldspace panel for editing scripts

@export var pointer_group: StringName = &"pointer_interactable"
@export var ui_size: Vector2 = Vector2(800, 600)
@export var quad_size: Vector2 = Vector2(3.2, 2.4)  # Aspect ratio matches 800:600
...
@onready var script_editor = get_node_or_null("SubViewport/ScriptEditorUI")
...
func open_script(path: String) -> void:
	if script_editor and script_editor.has_method("open_script"):
		script_editor.open_script(path)
		# Focus the editor after opening
		if script_editor.has_method("focus_editor"):
			script_editor.focus_editor()


## Open a script resource directly
func open_script_resource(script: Script) -> void:
	if script_editor and script_editor.has_method("open_script_resource"):
		script_editor.open_script_resource(script)
		if script_editor.has_method("focus_editor"):
			script_editor.focus_editor()
...
func handle_pointer_event(event: Dictionary) -> void:
	...
	if debug_coordinates:
		print("ScriptEditorViewport: Hit uv=", uv)
	...
	match event_type:
		...
		"press":
			...
			if script_editor and script_editor.has_method("focus_editor"):
				script_editor.focus_editor()
		"hold":
			_send_mouse_motion(viewport_pos)
...
func _send_mouse_motion(pos: Vector2) -> void:
	...
	if _is_pressed:
		motion_event.button_mask = MOUSE_BUTTON_MASK_LEFT as MouseButtonMask
...
func _send_mouse_button(pos: Vector2, pressed: bool, just_changed: bool, button_index: int = MOUSE_BUTTON_LEFT) -> void:
	...
	if pressed:
		if button_index == MOUSE_BUTTON_RIGHT:
			button_event.button_mask = MOUSE_BUTTON_MASK_RIGHT as MouseButtonMask
		else:
