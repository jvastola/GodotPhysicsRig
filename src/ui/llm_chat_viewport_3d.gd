...
# LLM Chat Viewport 3D - 3D worldspace panel for LLM chat terminal

@export var pointer_group: StringName = &"pointer_interactable"
@export var ui_size: Vector2 = Vector2(800, 600)
@export var quad_size: Vector2 = Vector2(3.2, 2.4)
...
func set_focus(focused: bool) -> void:
	...
	if focused and viewport:
		# Don't auto-focus a specific control - let the click handle it
		pass


func _focus_control_at_position(pos: Vector2) -> void:
	if not viewport:
		return
	
	# Find the control at the click position and focus it
	var chat_ui = viewport.get_node_or_null("LLMChatTerminalUI")
	if not chat_ui:
		return
	
	# Recursively find focusable control at position
	var control := _find_focusable_at_pos(chat_ui, pos)
	if control:
		control.grab_focus()


func _find_focusable_at_pos(node: Control, pos: Vector2) -> Control:
	if not node.visible:
		return null
	...
	for i in range(node.get_child_count() - 1, -1, -1):
		...
		if child is Control:
			var result := _find_focusable_at_pos(child, pos)
			if result:
				return result
	
	# Check if this node contains the position and is focusable
	var rect := node.get_global_rect()
	if rect.has_point(pos):
		if node.focus_mode != Control.FOCUS_NONE:
			if node is LineEdit or node is TextEdit or node is SpinBox:
				return node
...
func handle_pointer_event(event: Dictionary) -> void:
	...
	if debug_coordinates:
		print("LLMChatViewport: Hit uv=", uv)
...
func _send_mouse_button(pos: Vector2, pressed: bool, just_changed: bool, button_index: int = MOUSE_BUTTON_LEFT) -> void:
	...
	if pressed and button_index == MOUSE_BUTTON_LEFT:
		set_focus(true)
		# Force GUI to update focus at click position
		if viewport.gui_get_focus_owner():
			# Let the viewport handle focus naturally
			pass
		else:
			# Try to find and focus the control at this position
			_focus_control_at_position(pos)
...
# ============================================================================
# LLM CHAT SPECIFIC METHODS
# ============================================================================

## Get the LLM chat UI instance
func get_chat_ui() -> PanelContainer:
	if viewport:
		return viewport.get_node_or_null("LLMChatTerminalUI")
	return null


## Send a message to the LLM
func send_message(message: String) -> void:
	var chat_ui = get_chat_ui()
	if chat_ui and chat_ui.has_method("send_message"):
		chat_ui.send_message(message)


## Set the API key
func set_api_key(key: String) -> void:
	var chat_ui = get_chat_ui()
	if chat_ui and chat_ui.has_method("set_api_key"):
		chat_ui.set_api_key(key)


## Check if currently streaming
func is_streaming() -> bool:
	var chat_ui = get_chat_ui()
	if chat_ui and chat_ui.has_method("is_streaming"):
		return chat_ui.is_streaming()
	return false
