...
# Follows the UIViewport3D pattern for pointer interaction

@export var pointer_group: StringName = &"pointer_interactable"
@export var ui_size: Vector2 = Vector2(600, 500)  # Match viewport/UI size
@export var quad_size: Vector2 = Vector2(2.4, 2.0)  # Aspect ratio matches 600:500
...
func handle_pointer_event(event: Dictionary) -> void:
	...
	if debug_coordinates:
		print("SceneHierarchyViewport: Hit: global=", hit_pos, " local=", local_hit, " uv=", uv)
