...
## FileSystem Viewport 3D - 3D worldspace wrapper for FileSystem browser

@export var pointer_group: StringName = &"pointer_interactable"
@export var ui_size: Vector2 = Vector2(500, 700)
@export var quad_size: Vector2 = Vector2(2.0, 2.8)  # Aspect ratio matches 500:700
...
@onready var file_system_ui = get_node_or_null("SubViewport/FileSystemUI")
...
func _ready() -> void:
	...
	if file_system_ui and file_system_ui.has_signal("close_requested"):
		file_system_ui.close_requested.connect(_on_close_requested)
...
func handle_pointer_event(event: Dictionary) -> void:
	...
	if debug_coordinates:
		print("FileSystemViewport: Hit uv=", uv)
...
func _send_mouse_button(pos: Vector2, pressed: bool, just_changed: bool, button_index: int = MOUSE_BUTTON_LEFT) -> void:
	...
	if pressed and button_index == MOUSE_BUTTON_LEFT:
		_handle_potential_double_click(pos)
...
func _handle_potential_double_click(pos: Vector2) -> void:
	var current_time = Time.get_ticks_msec() / 1000.0
	var time_diff = current_time - _last_click_time
	var pos_diff = (pos - _last_click_pos).length()
	
	if time_diff < DOUBLE_CLICK_TIME and pos_diff < DOUBLE_CLICK_DIST:
...
func refresh() -> void:
	if file_system_ui and file_system_ui.has_method("refresh"):
		file_system_ui.refresh()
