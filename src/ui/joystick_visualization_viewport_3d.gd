...
# Joystick Visualization Viewport 3D

@export var pointer_group: StringName = &"pointer_interactable"
@export var ui_size: Vector2 = Vector2(600, 350)
@export var quad_size: Vector2 = Vector2(0.6, 0.35)
...
func _ready() -> void:
	...
	if viewport:
		...
		# Connect close signal from UI if needed
		# var ui = viewport.get_node_or_null("JoystickVisualizationUI")
	...
	if mesh_instance and _static_body:
		...
		# Manually create and bind material to ensure it works
		var mat := StandardMaterial3D.new()
		mat.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
		mat.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED
		mat.albedo_color = Color.WHITE # Base color
		...
		if viewport:
			...
			if tex:
				...
				print("JoystickVisViewport: ERROR - Viewport has no texture!")
				mat.albedo_color = Color(1, 0, 0, 1) # Red error
		else:
			mat.albedo_color = Color(1, 0, 0, 1) # Red error
			
		mesh_instance.material_override = mat
		print("JoystickVisViewport: Material setup complete. Viewport: ", viewport)
...
func handle_pointer_event(event: Dictionary) -> void:
	...
	if debug_coordinates:
		print("JoystickVisViewport: Hit uv=", uv)
