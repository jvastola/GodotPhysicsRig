extends Grabbable
class_name TwoHandGrabCube

# Two-Hand Grab Cube
# Allows simultaneous grabbing with two pointers to scale and rotate

@export_group("Two Hand Settings")
@export var scale_sensitivity: float = 1.0
@export var rotation_sensitivity: float = 1.0
@export var smoothing: float = 10.0 # Lerp speed
...
# Grab reference data
var _initial_cube_transform: Transform3D
...
func _create_indicators() -> void:
	...
	# Indicators need to be top-level to stay at world coordinates easily
...
func _physics_process(delta: float) -> void:
	...
	# If we are not grabbing, we look for start condition
	if not _is_two_hand_grabbing:
		...
		if left_trigger and left_hit: _left_pressing_me = true
		if not left_trigger: _left_pressing_me = false
		
		if right_trigger and right_hit: _right_pressing_me = true
		if not right_trigger: _right_pressing_me = false
...
func _start_two_hand_grab() -> void:
	...
	_initial_cube_transform = global_transform
	...
	if debug_draw:
		print("TwoHandGrab: Started. Dist: ", _initial_distance)
...
func _process_two_hand_grab(delta: float) -> void:
	...
	if lock_y_axis:
		# Use horizontal distance only for scaling if locked? 
		# Or maybe keep full distance for scaling but lock rotation/pos?
		# Usually scaling should be consistent with visual indicators.
		# If we lock Y rotation, maybe we should scale based on planar distance?
		# Let's keep 3D distance for scaling for now, usually intuitive enough.
		pass
	...
	var new_scale = _initial_cube_transform.basis.get_scale() * scale_ratio
	# Clamp scale
	new_scale = new_scale.clamp(Vector3.ONE * min_scale, Vector3.ONE * max_scale)
	
	# 2. Rotation
	# We want the rotation that transforms initial_vector to current_vector
	...
	# The axis is the cross product, angle is the angle between
	...
	# Start with initial
	var target_basis = rot_diff * _initial_cube_transform.basis
	# Apply scale
	target_basis = target_basis.orthonormalized().scaled(new_scale)
	
	# Rotation pivot logic:
	var offset_from_mid = _initial_cube_transform.origin - initial_mid
	var rotated_offset = rot_diff * offset_from_mid
	var target_origin = current_mid + rotated_offset
	
	if lock_y_axis:
		target_origin.y = _initial_cube_transform.origin.y
	
	# Apply smoothing
	var t = Transform3D(target_basis, target_origin)
	global_transform = global_transform.interpolate_with(t, smoothing * delta)
...
func _check_pointer_hit(pointer: Node3D) -> bool:
	# This requires the pointer to expose its hit status or we check distance
	# Simply checking distance to the hit point is unreliable if we are big
	# Best is to check if `hand_pointer.get_hit_point()` is close to us?
	# Or better, check if the ray passes through us. 
	# Since hand_pointer does raycasting, we can't easily ask it "did you hit ME?".
	# But `handle_pointer_event` is called when it hits us.
	# We can use a flag set in `handle_pointer_event`.
	return _is_pointer_hitting_me(pointer)

# Helper to verify hit
func _is_pointer_hitting_me(pointer: Node3D) -> bool:
	# We can cheat and just check if the get_hit_point() is close to our collider surface
	# Or check if pointer.get_hit_collider() == self (if we could add that accessor)
	# Let's trust `handle_pointer_event` to verify hits? 
	# Problem: `handle_pointer_event` is event driven.
	# Let's add `get_hit_collider()` to hand_pointer as well?
	# Or just raycast ourselves? No, duplicate work.
	
	# Let's add `get_hit_collider()` to hand_pointer.gd, it's safer.
	if pointer.has_method("get_hit_collider"):
		var col = pointer.get_hit_collider()
		return col == self or (col is Node and self.is_ancestor_of(col))
	return false
