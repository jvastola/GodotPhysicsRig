...
if (-not $SkipPlugins) {
    ...
    if (Test-Path $LIVEKIT_DIR) {
        ...
        if (Test-Path "build\outputs\aar\livekit-release.aar") {
            Copy-Item "build\outputs\aar\livekit-release.aar" "..\..\android\plugins\GodotLiveKit.aar" -Force
            Write-Host "✓ GodotLiveKit plugin built" -ForegroundColor Green
        } elseif (Test-Path "build\outputs\aar\GodotLiveKit-release.aar") {
            Copy-Item "build\outputs\aar\GodotLiveKit-release.aar" "..\..\android\plugins\GodotLiveKit.aar" -Force
            Write-Host "✓ GodotLiveKit plugin built" -ForegroundColor Green
        } else {
            Write-Host "WARNING: GodotLiveKit AAR not found, checking for existing..." -ForegroundColor Yellow
            if (Test-Path "..\..\android\plugins\GodotLiveKit.aar") {
        ...
        if (Test-Path "GodotLiveKit.gdap") {
            Copy-Item "GodotLiveKit.gdap" "..\..\android\plugins\" -Force
        }
